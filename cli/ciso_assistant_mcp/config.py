...
__all__ = ['make_request', 'format_table', 'API_URL', 'TOKEN', 'VERIFY_CERTIFICATE']
    ...
    try:
        if method.upper() == "GET":
            ...
            response = requests.delete(
                url, headers=headers, verify=VERIFY_CERTIFICATE, timeout=30
            )
        else:
        ...
        for col in columns:
            ...
            if isinstance(value, dict):
                ...
                if len(item.get(col, [])) > 3:
                    value += "..."
            row_data.append(str(value).replace("|", "\\|"))  # Escape pipes
